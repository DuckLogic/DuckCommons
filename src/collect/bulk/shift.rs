...
/// but also consider our own range of intialized memory `[shifted_start, shifted_end]`.
...
impl<'a, T: 'a> BulkShifter<'a, T> {
    ...
    pub fn shift_original(&mut self, start: usize) {
        ...
        assert!(
            self.shifted_start >= moved_memory
                && start <= self.shifted_start - moved_memory,
            "Insufficient room to move from {}: {:?}", start, maybe_debug!(self)
        );
        unsafe {
            ptr::copy(
    ...
    pub fn finish(self) -> &'a mut Vec<T> {
        assert!(self.is_finished(), "Unfinished: {:?}", maybe_debug!(&self));
        unsafe { self.target.set_len(self.shifted_end); }
        self.target
    }
